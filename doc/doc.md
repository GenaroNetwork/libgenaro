...
* 设置环境变量GENARO_CURL_OUT_DIR为某个目录，可以将curl进行http通讯的过程打印到相应目录的_genaro_curl_debug.log文件中。
...
## AES加密和Reed-Solomn
  ...
  文件的加密用的是AES-256-CTR对称加密算法，文件的纠错用的是FEC编码算法中的Reed-Solomn算法（上传时传递的rs参数为true时才会使用此算法）：
  1. 如果文件小于或等于MIN_SHARD_SIZE，不使用Reed-Solomn算法，并会在对每个shard进行prepare_frame时直接加密后上传，所以上传的数据大小就是原始文件大小。
  2. 如果文件大于MIN_SHARD_SIZE，那么上传文件前会在系统的临时目录中生成以".crypt"为后缀的文件，该文件是通过AES-256-CTR加密的文件，然后生成以".parity"为后缀的文件，该文件是Reed-Solomn算法生成的文件（目前对2/3的shards进行了冗余，即该文件大小为源文件的2/3左右），上传时直接读取这两个文件的内容后上传，所以上传的数据大概是5/3原始文件大小。
  3. 下载完total_data_shards个shards时，中断下载，并使用Reed-Solomn算法恢复文件。
...
4. recover_shards：如果文件上传时使用了Reed-Solomon算法，那么在下载完total_data_shards个shards时，中断下载，并用Reed-Solomn算法恢复数据并进行解密；如果没有使用Reed-Solomon算法，那么当所有shards下载完后，进行解密和truncate操作。

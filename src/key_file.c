...
int extract_key_file_obj(const char *passphrase, key_file_obj_t *key_file_obj, key_result_t **ptr_key_result) {
    ...
    salt_hex = str_decode_to_hex(strlen(salt), salt);
    ...
    ciphertext_raw = str_decode_to_hex(ciphertext_len, ciphertext_str);
    ...
    char *str_mac_sha3 = hex_encode_to_str(CRYPTO_SHA3_DIGEST_SIZE, buf_mac_sha3);
    ...
    uint8_t *iv_raw = str_decode_to_hex(strlen(iv_str), iv_str);
